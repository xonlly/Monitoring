...
var OsMonitor = function () {
  function OsMonitor() {
    ...
    if (os.networkInterfaces && Object.keys(os.networkInterfaces()).length) {
      ...
      traffic(Object.keys(os.networkInterfaces()), function (traffic) {

        if (platform != 'win32') {
          _this.traffic = traffic;
        } else {
          _this.traffic = traffic.filter(function (elem, index) {
      ...
      var liveCpu = function liveCpu() {
        ...
        setInterval(function () {
          ...
          if (moyenneCpuArr.length > 5) {
            _this2.currentCPU = moyenneCpu(moyenneCpuArr);
