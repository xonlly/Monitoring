...
module.exports = function (ifaces, tick) {
    ...
    return new Promise((resolve, fail) => {
      ...
      if (os.platform() == 'win32') {
        ...
        new Download({mode: '755'})
          .get('http://gael.roualland.free.fr/ifstat/ifstat-1.1-win32.zip')
          .dest( __dirname + '/win32')
          .run(( err, files ) => {
            if (err) {
              Log.err(err)
              fail();
      ...
      child_process.exec('command -v ifstat', (error, stdout, stderr) => {
        if (stdout == '') {
          return fail();
        } else {
          return resolve();
  ...
  ifstatExist().then(() => {
    ...
    ifstat.stdout.on('data', (data) => {
      if (!firstTick) {
        return tick(chunk(data.toString().trim().split(/\s/g).filter((line) => {
          return line !== '';
        }), 2));
      }
      firstTick = false;
